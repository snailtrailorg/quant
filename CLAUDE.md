...
- **单系统 RBAC（2026-09-09 批11B 动态用户组）**：四内置组 Admin/Trader/Analyst/Viewer（`user_group` 表 builtin=true **锁名不可删不可改名**）+自定义组可增删改（新组零权限起步），非多租户。权限真源=`permission` 表（role|user × nav|api|data × allow|deny），**join 键=组名字符串**（require_perm/load_effective_permissions 零依赖组 id）；`load_effective_permissions`（user deny > user allow > role allow）；**身份与权限分层**：身份类 7 端点（me/logout/profile/avatar/deactivate/change-password）走 `require_authenticated`（已认证+账号有效即可，不查权限——自定义组零权限也须可用），功能端点 `require_perm` 全站统一+DB role（降级即时生效）；锁键 {user_mgmt,resume,account_keys} 双路径同锁+admin 地板（任意非 admin 组恒不可授予）；组 rename 三表级联（user_group/permission/users）+invalidate 缓存，删组级联清权限+软删用户归 viewer；数据脱敏 `data_sensitivity`→detail|aggregated|count 七端点；菜单由 perms 驱动+nav 维守卫（hidden 拒/readonly 灰）；玻璃盒 perm_sources。管理=系统管理→用户管理（用户列表/用户群组双页签，组弹窗含三维矩阵 PermMatrix 同源组件；设置→权限管理矩阵页保留对照）。
