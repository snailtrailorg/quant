...
- 共享行情 Hub：**终态设计=`docs/design/D26-市场接入架构.md`**（账号级 hub 拓扑+进程模型统一+M5 退役；hub=纯数据面 MD、worker=TD-only、Valkey Streams 分发+boot 守卫/gen）；旧需求/设计（D13/D14 单活双实例形态）已归档 obsolete；现行代码形态见模块契约/md_hub.md
