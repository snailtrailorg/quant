...
- **外部接口配置模型**（2026-09-19 用户裁定，详 A02架构文档；D25 能力集重构 2026-09-25 上产，详 `docs/design/D25-三层能力模型.md`）：行的边界=账号（连接身份）/列语义=能力集/集成中心=**单列表+能力筛选**（页签合并——多能力接口 XTP/币安只出现一次）。统一表 `external_interface`（55a/55b 已落地，旧两表已删）；能力集 5 token=`hist_quote 历史行情/rt_quote 实时行情/trading 交易/ref_data 参考数据/inst_event 标的事件`（**trading 保词**——8 处 SQL 域谓词零改动；真源=`markets.py KIND_CAP_CLASS` 从 DataKind 27 词派生，全覆盖互斥测试断言）。能力真源=代码 adapter（配置列恒=启用子集，写侧 ⊆ 校验）；XTP 等多能力接口单份配置两能力可见。**配置存放立法**：`.env` 只放加密密钥（SECRET_KEY 类），其余所有配置入库，敏感值用密钥加密后入库（credentials_encrypted）；鸡生蛋例外=DB/Valkey/Celery 连接串；迁移次序=先建库行验证再清 .env 值。
...
- **产出落文件**，不留在对话里。
- **给用户的命令必带五要素**：机器/账号/密码（写指针如记忆 server-info，禁明文）/前置（2026-08-30 三次环境不匹配教训）。
...
- **待办自包含**：新待办条目写在 `flow/待办.md`（未完成任务**索引**，引用任务文档），任务文件按 `flow/规范/任务模板.md` 8 字段写、放 `flow/任务/`（进行中，批号命名 `批xxx-描述.md`，引用相关知识文档），做任务时只读「任务文件 + `docs/architecture/接口契约.md` + 本模块契约 `docs/architecture/模块契约/<module>.md`」，零代码阅读。硬约束：限定范围 ≤3 文件 + 参考 ≤2 份。完成后任务文件归档 `docs/obsolete/任务归档/`。
...
- **市场接入架构（D26 v5 终审定稿，挂起待实施）**：`docs/design/D26-市场接入架构.md`——全市场统一账号级 hub 拓扑（推翻 A05 A股单 hub；一行=一账号=一 hub+一 TD 会话）+M5 互备退役+hub 进程模型统一（常驻+连接窗=内部状态机）；实施=下次接入面动工触发，按 §五 13 项迁移清单展开
...
- 共享行情 Hub（ST7，⚠️ hub 形态已被 D26 推翻待实施）：`docs/design/D13-共享行情hub需求书.md` + `D14-共享行情hub设计.md` v2（hub=纯数据面 MD 单活双实例+active_instance 仲裁+批 60 M5 切换协议——**D26 终态=全市场账号级 hub 拓扑+M5 退役**，现行代码仍为旧形态；worker=TD-only；Valkey Streams 分发+租约 fencing）
