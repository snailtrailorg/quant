...
- **市场接入架构（D26 v5 终审定稿，挂起待实施）**：`docs/design/D26-市场接入架构.md`——全市场统一账号级 hub 拓扑（一行=一账号=一 hub+一 TD 会话；设计取舍见其 §零）+M5 互备退役+hub 进程模型统一（常驻+连接窗=内部状态机）；实施=下次接入面动工触发，按 §五 13 项迁移清单展开
