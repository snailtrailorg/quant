...
- **单系统 RBAC（批33a 权限单源化 2026-09-17）**：四内置组 Admin/Trader/Analyst/Viewer（`user_group` 表 builtin=true **锁名不可删不可改名**）+自定义组可增删改（新组零权限起步），非多租户。权限真源=`permission` 表（**subject_type='role' 单维**——迁移 0082 CHECK 锁值域，user 覆盖维整体退役：权限完全追随组，新组合=建新组；生产 user 行复核=0 直接清），维度=nav|api|market_op × allow|deny，**join 键=组名字符串**（require_perm/load_effective_permissions 零依赖组 id）；`load_effective_permissions`=角色单源（签名+返回形状保持——perm_sources 恒 role-base/__denied__ 恒空，玻璃盒/IM 链/测试 patch 零改动）；**身份与权限分层**：身份类 7 端点（me/logout/profile/avatar/deactivate/change-password）走 `require_authenticated`（已认证+账号有效即可，不查权限——自定义组零权限也须可用），功能端点 `require_perm` 全站统一+DB role（降级即时生效）；锁键 {user_mgmt,resume,account_keys} 组层单锁+admin 地板（任意非 admin 组恒不可授予；user 写点随端点退役=唯一绕过面封死）；组 rename 三表级联（user_group/permission/users）+invalidate 缓存，删组级联清权限+软删用户归 viewer；菜单由 perms 驱动+nav 维守卫（hidden 拒/readonly 灰）；管理=系统管理→用户管理（用户列表/用户群组双页签，组弹窗含三维矩阵 PermMatrix 同源组件——**权限配置唯一入口**；设置→权限管理页已退役 redirect /users?tab=groups）。
