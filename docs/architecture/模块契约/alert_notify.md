...
> **批48（2026-09-18）**：停用 bot 语义——avail 含停用（显示面全通道恒显）；勾选**自有停用→保留落库**（`:223` 发送+`:376` 订阅展开双 enabled 过滤天然不发，bot 复启自动恢复发送）；原 own_disabled 死分支（同条件二查恒 false）致停用勾选 400 错报已修。
> **批43（2026-09-18）短信多行容灾**：`sms_provider` 表（迁移 0086——每行一个服务商通道：凭证+签名+两模板编号+position 单层序+enabled；存量五键随迁+DELETE）；`_available_providers(require_verify_tpl)` 候选集对称（告警=四件套+alert_tpl/验证码=四件套+verify_tpl，两集独立）；`_send_with_failover` 逐行尝试（**上限 3 行+per-row timeout (2,5)+分类表**：ALIYUN_*/5xx/ConnectError→unreachable 切次行；timeout 模糊结局不切防双发计费）；`send_sms/send_sms_code` 签名不动（dispatch/phone 链零改）。CRUD+reorder 端点族（alerts_config 门）；`/api/alerts/sms-config` GET·PUT 退役；`soft_time_limit` 15→30。前端 SmsCard=通道表格（拖拽手柄 HTML5 原生+页签改名「短信通道」）。
