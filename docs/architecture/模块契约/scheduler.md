...
- **批25（2026-09-15）**：system_log 落库 celery 侧装配（`setup_logging` 挂回 root + `worker_process_init` 每子进程自起 flush 线程 + `worker_process_shutdown` 子进程尾窗冲刷）。详见 `docs/architecture/模块契约/` 各模块与 `docs/任务/批25-全局日志三套体系.md`。
