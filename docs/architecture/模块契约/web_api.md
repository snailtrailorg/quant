...
> **最近变更（2026-09-24 多账号源 D5）**：create_live_task 三级时点① `account_allows(account_id, symbol)`（routes/trading.py）；建任务 account_id 必填+int 校验。
...
- **告警订阅（批30 用户化/批34 通道级/批39 webhook 退役/批48 全通道恒显）**：GET `/api/alerts/config`（行 channels_sel 剥离失效键+channels_avail 用户三通道明细——**批48 avail.bots 含停用 bot 带 enabled 标志**，前端四态 chip 恒显）；POST/PUT `/api/alerts/config{/{id}}`（channels 三态 null/[]/勾选——`_validate_channels` 缺键=沿用/自有停用**保留**（批48，dispatch enabled 过滤不发复启自动恢复）/实体消失剥离/不属用户 400）；DELETE；POST `/api/alerts/test`（按勾选过滤 60s 冷却）；`/api/alerts/sms-config` GET·PUT（**批38 三段语义**：缺键=不改/密钥空=不改/明文三键空=真清空——所见即所得；批37 从告警页迁集成中心页签）
...
/api/interfaces` `?cap=` `{iid}/{test}` `/api/interfaces/reorder` `/api/interfaces/providers` | GET/POST/DELETE | admin | 批55a：统一表 CRUD+分域 reorder（行=账号/列=能力——27 号）；GET 按能力过滤（GIN @>）+漂移告警+附 code_capabilities；批55b：providers=新建下拉目录（注册表派生） | GET/POST | viewer+ | PT1（list/get/终止/强删/卡死检测） |
| 消息通道 | `/api/channels` `/api/channels/{cid}/{test}` | GET/POST/POST/DELETE | admin | PT4（channel_config CRUD） | GET/POST/POST/DELETE | admin | PT6（risk_rules CRUD） |
...
| 策略-账户绑定 | `/api/strategy_account` `/api/strategy_account/{said}` | GET/POST/DELETE | viewer+ / strategy_control | #27 绑定关系 CRUD | GET/POST | viewer+ | 通知中心（active 历史 + 全部已读） |
