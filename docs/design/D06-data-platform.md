...
> 原编号 06（2026-09-24 文档体系重构，architecture→design 拆分）。

> **三档数据供给（2026-08-19 U 审定稿）**：选股/深度/详情页数据按消费模式分三档（全局批量 9 表 + 池内 per-symbol 10 表 + 按需实时不落库），一档二档已上线。详见 [17-三档数据与详情页](D17-三档数据与详情页.md)。
>
...
> 来源：`docs/reference/多资产量化数据中台架构升级方案.md` 评审（对 60%/错 40%/过度 100%）——三件做、三件不做有理由。设计决策详规：`docs/obsolete/任务归档/限流治理吸收.md`（D1-D3）。
