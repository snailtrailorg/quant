# D13 · 共享行情 Hub 架构 · 稳定性需求书（P-6 交付物 · 2026-08-17）
...
> 原编号 13（2026-09-24 文档体系重构，architecture→design 拆分）。

> ST7 的设计门禁。来源：稳定性检查矩阵 F-9~F-12、盲审报告 §4、方法论 §6-3、SA-SE 五批次实施经验。
> 每条需求带验收判据（可证伪）。设计（14 号文档）与实施（ST7）逐条对照本文，缺一条即不通过评审。
>
> **后记（2026-09-19）**：影子期已结束（bar_shadow 冻结为历史表）、direct 模式已退役（批 6b 收官——hub 模式唯一，md_mode=direct→EX_CONFIG）；hub 侧 gen/租约机制的理想态演进见 **28 号 §八**。
> **当前状态（2026-08-27，历史）**：hub 已在产（批 2 主循环迁 runtime/ 骨架）；R-BR20 影子门禁处于
