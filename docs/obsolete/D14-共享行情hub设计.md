# D14 · 共享行情 Hub 架构 · 设计 v2（ST7 · 2026-08-17）

> ⚠️ **D26 部分推翻（2026-09-25 终审，挂起待实施）**：本文档的「单活双实例（A 现任+B 按需）+ active_instance 仲裁 + M5 切换协议」形态已被 `D26-市场接入架构.md` 推翻——全市场统一**账号级 hub 拓扑**（每账号一个 hub，无共享单例/无互备），M5 协议整体退役（守卫 a′ 保留）。**现行生产代码仍是本文档形态**；D26 实施批（触发=下次接入面动工）切换后本文档随批归档。

> 原编号 14（2026-09-24 文档体系重构，architecture→design 拆分）。
...
> **M5 演进注记（批 60 v15，2026-09-22）**：单实例租约防脑裂演进为**单活双实例切换协议**（A 现任 + B 按需启动 + active_instance 仲裁）——hub 仍是单活（任一时刻至多一个写者），但支持计划性切换供应商账号。§2.4/2.5/4 已加注；协议全貌见 `docs/obsolete/任务归档/批60-方案集.md` v15（20+ 轮评审收敛：Valkey 四键 + guarded Lua 条件推进 + boot 单判定，砍 stream_registry/影子预热/fencing token/待命模式）。
