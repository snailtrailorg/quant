...
# ─── IM 统一接入（19 号 v2 批 2：/api/im-bots 通用端点群，替换旧 /api/feishu/*） ───

@app.get("/api/im-bots/providers")
...
@app.post("/api/im-bots/onboarding/{provider}")
...
@app.get("/api/im-bots/onboarding-status/{ticket}")
...
@app.post("/api/im-bots/{bid}/start")
...
class IMBotUpdateReq(BaseModel):
    ...
    lang: str | None = None
    credentials: dict | None = None     # 部分 fields 补录(如 verification_token)


@app.post("/api/im-bots/{bid}")
...
# ── 用户授权管理面(批 2,19 号 v2 §1 授权决策链的管理入口) ──

@app.get("/api/im-bots/{bid}/users")
...
@app.post("/api/im-bots/{bid}/users")
...
@app.delete("/api/im-bots/{bid}/users/{im_user_id}")
...
@app.get("/api/sync/pool-minute/progress")
