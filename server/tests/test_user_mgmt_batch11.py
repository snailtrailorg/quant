...
class _SqlConn:
    """update_user 专用：按 SQL 前缀路由返回（SELECT 用户/查重 vs UPDATE）。批24 迭代八。"""

    def __init__(self, user_exists=True, email_taken=False):
        self.user_exists, self.email_taken = user_exists, email_taken
        self.executed = []
    ...
    def execute(self, sql, params=None):
        ...
        if sql.startswith("SELECT username FROM users"):
            cur.fetchone.return_value = ("bob",) if self.user_exists else None
        elif "AND id<>" in sql:   # email 查重
            cur.fetchone.return_value = ("x",) if self.email_taken else None
        elif sql.startswith("SELECT 1 FROM user_group"):
            cur.fetchone.return_value = (1,)
        return cur


class TestUpdateUserEmail:
    """批24 迭代八：POST /api/user/{uid} email 参数（格式 400/占用 409/成功 SQL+审计）。"""

    def _post(self, qs, conn):
        with patch("src.web_api.auth.verify_jwt", return_value=ADMIN), \
             patch("src.web_api.routes.auth_routes.get_conn", return_value=conn), \
             patch("src.data_platform.db.get_conn", return_value=conn), \
             patch("src.web_api.routes.auth_routes.audit_log") as au:
            r = _client().post(f"/api/user/5?{qs}", headers={"Authorization": "Bearer t"})
        return r, au

    def test_email_invalid(self):
        r, _ = self._post("email=not-an-email", _SqlConn())
        assert r.status_code == 400 and r.json()["code"] == "EMAIL_INVALID"

    def test_email_taken(self):
        r, _ = self._post("email=taken@x.com", _SqlConn(email_taken=True))
        assert r.status_code == 409 and r.json()["code"] == "EMAIL_TAKEN"

    def test_email_update_sql_and_audit(self):
        conn = _SqlConn()
        r, au = self._post("role=trader&enabled=true&email=New@Example.com ", conn)
        assert r.status_code == 200 and r.json() == {"ok": True}
        ups = [s for s, _ in conn.executed if s.startswith("UPDATE users SET email")]
        assert ups and ups[0].count("%s") == 2           # 参数化（email, uid）
        # strip+lower 归一后入库/审计（批20 同规范）
        assert ("UPDATE users SET email=%s WHERE id=%s", ("new@example.com", 5)) in conn.executed
        detail = au.call_args[0][-1]
        assert "email=new@example.com" in detail
