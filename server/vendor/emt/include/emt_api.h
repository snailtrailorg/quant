...
/*******************************************************************回调函数定义*******************************************************************/
    ...
    ///查询客户可用接入点ID响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    ///@param endpoint_info 可用接入点信息
    ///@param error_info 查询昨日资产发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
    ///@param request_id 此消息响应函数对应的请求ID
    ///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    typedef void (*EMT_OnQueryEndpointId)(EMTQueryEndpointIdRsp *endpoint_info, EMTRI *error_info, int request_id, bool is_last, uint64_t session_id);
    ...
    ///分页请求查询黑白名单，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    ///@param bwl_info 查询到的一个黑白名单信息
    ///@param req_count 请求到的最大数量
    ///@param rsp_sequence 分页请求的当前回报数量
    ///@param query_reference 当前报单信息所对应的查询索引，需要记录下来，在进行下一次分页查询的时候需要用到
    ///@param request_id 此消息响应函数对应的请求ID
    ...
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 当rsp_sequence为0，表明当次查询没有查到任何记录，当is_last为true时，如果rsp_sequence等于req_count，那么表示还有回报，可以进行下一次分页查询，如果不等，表示所有回报已经查询完毕。一个查询请求可能对应多个响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线。
    typedef void (*EMT_OnQueryBlackWhiteListByPage)(EMTQueryBlackWhiteListRsp *bwl_info, int64_t req_count, int64_t rsp_sequence, int64_t query_reference, int request_id, bool is_last, uint64_t session_id);

    ///融资融券两地分仓保证金调拨通知
    ///@param marginavl_transfer_info 保证金调拨通知的具体信息，用户可以通过marginavl_transfer_info.serial_id来管理订单，通过GetClientIDByEMTID() == client_id来过滤自己的订单。
    ///@param error_info 保证金调拨订单被拒绝或者发生错误时错误代码和错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误。
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 当保证金调拨订单有状态变化的时候，会被调用，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线。所有登录了此用户的客户端都将收到此用户的调拨通知。
    typedef void (*EMT_OnCreditMarginAvlTransfer)(EMTMarginAvlTransferNotice *marginavl_transfer_info, EMTRI *error_info, uint64_t session_id);
    
    ///请求查询融资融券两地分仓保证金调拨订单响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    ///@param marginavl_transfer_info 保证金调拨通知的具体信息
    ///@param error_info 查询保证金发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
    ///@param request_id 此消息响应函数对应的请求ID
    ///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    typedef void (*EMT_OnQueryCreditMarginAvlTransfer)(EMTMarginAvlTransferNotice *marginavl_transfer_info, EMTRI *error_info, int request_id, bool is_last, uint64_t session_id);
    ...
    EMTAPI_EXPORTS int  RegistCallBack(
        ...
        EMT_OnQueryFund                       cb63, EMT_OnQueryEndpointId                 cb64, EMT_OnQueryDMATickerAssignInfoByPage  cb65, EMT_OnQueryBlackWhiteListByPage         cb66,
        EMT_OnCreditMarginAvlTransfer         cb67, EMT_OnQueryCreditMarginAvlTransfer    cb68);
    ...
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@remark 此函数支持一号两中心节点之间的信用额度划拨，注意划拨的方向。
    ...
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@param request_id 用于用户定位查询响应的ID，由用户自定义
    EMTAPI_EXPORTS int QueryEndpointId(const EMTQueryEndPointIdReq *query_param, uint64_t session_id, int request_id);
    ...
    ///请求查询黑白名单信息分页
    ///@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
    ///@param query_param 需要分页查询查询黑白名单信息的条件，如果第一次查询，那么query_param.reference填0
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@param request_id 用于用户定位查询响应的ID，由用户自定义
    EMTAPI_EXPORTS int QueryBlackWhiteListByPage(const EMTQueryBlackWhiteListByPageReq *query_param, uint64_t session_id, int request_id);

    ///融资融券两地分仓保证金调拨请求
    ///@return 保证金调拨订单在EMT系统中的ID,如果为“0”表示消息发送失败，此时用户可以调用GetApiLastError()来获取错误代码，非“0”表示消息发送成功，用户需要记录下返回的serial_id，它保证一个交易日内唯一，不同的交易日不保证唯一性
    ///@param marginavl_transfer 保证金划拨的请求信息
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@remark 此函数支持一号两中心节点之间的保证金划拨，注意划拨的方向。
    EMTAPI_EXPORTS uint64_t CreditMarginAvlTransfer(EMTMarginAvlTransferReq *marginavl_transfer, uint64_t session_id);

    ///融资融券两地分仓保证金调拨查询请求
    ///@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
    ///@param query_param 需要查询的保证金调拨订单筛选条件，其中serial_id可以为0，则默认所有资金划拨订单，如果不为0，则请求特定的保证金划拨订单
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@param request_id 用于用户定位查询响应的ID，由用户自定义
    EMTAPI_EXPORTS int QueryCreditMarginAvlTransfer(EMTQueryMarginAvlTransferLogReq *query_param, uint64_t session_id, int request_id);
#ifdef __cplusplus
}
#endif
