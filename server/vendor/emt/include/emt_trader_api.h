...
namespace EMT {
    namespace API {
        ...
        public:
            ...
            ///@return 创建出的UserApi
            ///@remark TraderApi为单例模式，重复调用返回相同的句柄，如需创建新的TraderApi,需要调用Release后重新调用CreateTraderApi。如果一个账户需要在多个客户端登录，请使用不同的client_id，系统允许一个账户同时登录多个客户端，但是对于同一账户，相同的client_id只能保持一个session连接，后面的登录在前一个session存续期间，无法连接。系统不支持过夜，请确保每天开盘前重新启动
