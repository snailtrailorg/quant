...
struct EMTQueryAssetRsp
{
    ...
    ///累计买入成交证券占用资金（仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用）
    double fund_buy_amount;
    ///累计买入成交交易费用（仅限现货账户/期权账户，现货账户含ETF申购等业务成交费用，信用账户暂不可用）
    double fund_buy_fee;
    ///累计卖出成交证券所得资金（仅限现货账户/期权账户，现货账户含ETF赎回等业务所得资金，信用账户暂不可用）
    double fund_sell_amount;
    ///累计卖出成交交易费用（仅限现货账户/期权账户，现货账户含ETF赎回等业务成交费用，信用账户暂不可用）
    ...
    ///查询类型,用于标注当前查询为何种类型的资金查询. 系统内部使用，用户无需关心
    int64_t query_type;
...
///两地分仓保证金内转流水通知
/////////////////////////////////////////////////////////////////////////
struct EMTMarginAvlTransferNotice
{
    ///内转编号
    uint64_t                emt_id;
    ///内转类型
    EMT_MARGINAVL_TRANSFER_TYPE    transfer_type;
    ///金额
    double                    amount;
    ///操作结果
    EMT_MARGINAVL_OPER_STATUS    oper_status;
    ///操作时间
    uint64_t                transfer_time;
};
...
///两地分仓保证金调拨流水查询请求与响应
/////////////////////////////////////////////////////////////////////////
struct EMTQueryMarginAvlTransferLogReq
{
    ///内转编号
    uint64_t    emt_id;
};
...
struct EMTCrdFundInfo
{
    ...
    double position_amount;         ///< 融资头寸可用金额
};
...
struct EMTClientQueryCreditTargetStkRsp
{
    ...
    EMT_CREDIT_FUND_CTRL_TYPE credit_fund_ctrl;         ///< 融资状态
    EMT_CREDIT_STK_CTRL_TYPE  credit_stk_ctrl;          ///< 融券状态
    double dbMarginRateFund;                            ///< 融资保证金比例
    double dbMarginRateStk;                             ///< 融券保证金比例
};
...
//查询黑白名单信息的请求-分页查询
struct EMTQueryBlackWhiteListByPageReq{
    bool is_black;                             ///< true - 黑名单； false - 白名单
    EMT_MARKET_TYPE market;                    ///< 证券市场
...
//查询黑白名单信息的响应
struct EMTQueryBlackWhiteListRsp {
    bool is_black;                              ///< true - 黑名单； false - 白名单
    EMT_MARKET_TYPE market;                     ///< 证券市场
    char ticker[EMT_TICKER_LEN];                ///< 证券代码
};
