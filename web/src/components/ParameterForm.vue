    ...
    <div v-for="(def, i) in defs" :key="i" style="margin-bottom: 12px">
      <el-form-item :label="def.label || def.name">
        <div style="display: flex; align-items: center; gap: 8px; width: 100%">
          ...
          <span style="color: var(--text-secondary); font-size: var(--fs-foot)">{{ def.description || '' }}</span>
        </div>
      </el-form-item>
    </div>
    <div v-if="!defs || defs.length === 0" style="color: var(--text-secondary); font-size: var(--fs-foot); padding-left: 100px">
      {{ t('common.noParams') }}
    </div>
  </div>
</template>
