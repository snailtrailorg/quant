...
const props = defineProps({
  ...
  routing: { type: Boolean, default: true },  // 批16 弹窗化：false=纯内部态（弹窗内用——不读不写 query，防与外层 ?profile= 互踩+污染深链）
})
...
const initial = () => {
  if (props.routing && route.query[props.queryKey]) return route.query[props.queryKey]
  if (props.memoryKey) {
...
const onTab = (v) => {
  ...
  if (props.routing) router.replace({ query: { ...route.query, [props.queryKey]: v } })
}
