export default {
  zh: {
    ...
    myIm: {   // 批11C：个人中心·我的 IM 通道；批11D 增方式/扫码
      ...
      qrIntro: '扫码接入：由飞书开放平台引导创建或连接应用，凭证自动配置（推荐）。',
      qrStart: '重新获取二维码', qrRetry: '重新获取二维码',
      qrValid: '二维码 {t} 内有效，过期后需重新获取',
      qrStarting: '正在生成二维码…', qrScanning: '请用手机飞书扫码，然后在手机上点击确认',
      qrDone: '接入成功', qrTimeout: '二维码已过期，请重新发起',
      qrConfirming: '手机已确认，正在生成验证码…',
      resumed: '已恢复进行中的接入会话',
      barHint: '将此验证码私聊发给新机器人完成绑定：',
      closeGuard: '正在等待手机确认/出码，取消后可从本页重新进入（会话仍在进行中）。确定取消？',
      boundTitle: '绑定成功！', boundGuide: '现在可以在飞书里和机器人对话了——试试发「查一下风控状态」',
      guideApp: '· 请用飞书 App 扫码（打开飞书 → 消息页右上角「+」→ 扫一扫）',
      guideChoice: '· 手机确认页若让你选择应用：之前建过此机器人选「连接现有应用」，第一次用选「重新创建应用」',
      guideWhy: '· 手机上的确认步骤来自飞书开放平台的授权流程，与本系统无关',
    ...
    imBots: {
      ...
      dingtalk: { step1: '钉钉开放平台（open-dev.dingtalk.com）→ 应用开发 → 创建企业内部应用',
                  step2: '应用内添加「机器人」能力，消息接收方式选「Stream 模式」并发布应用',
                  step3: '在「凭证与基础信息」页复制 AppKey / AppSecret 填入上方表单' },
      wecom: { step1: '企业微信管理后台 → 安全与管理 → 管理工具 → 智能机器人',
               step2: '创建机器人：手动创建 → API 模式 → 长连接（WebSocket）',
               step3: '在机器人详情页复制 BotId / Secret 填入上方表单' },
    },
    llm: {
      title: 'AI模型',
      budget: '预算预警',
  ...
  en: {
    ...
    myIm: {   // batch 11C: profile · my IM channels; batch 11D adds methods/QR
      ...
      qrIntro: 'QR onboarding: the Feishu open platform guides app creation/connection; credentials auto-configured (recommended).',
      qrStart: 'Get new QR code', qrRetry: 'Get new QR code',
      qrValid: 'QR code valid for {t}; get a new one after expiry',
      qrStarting: 'Generating QR code…', qrScanning: 'Scan with the Feishu app, then confirm on your phone',
      qrDone: 'Connected', qrTimeout: 'QR code expired — start again',
      qrConfirming: 'Confirmed on phone, generating verification code…',
      resumed: 'Resumed your active onboarding session',
      barHint: 'Send this code to the new bot in a private chat to bind:',
      closeGuard: 'Still waiting for phone confirmation / QR. You can re-enter from this page later. Cancel anyway?',
      boundTitle: 'Bound!', boundGuide: 'You can now chat with the bot in Feishu — try "check risk status"',
      guideApp: '· Scan with the Feishu app (Feishu → Messages → "+" (top-right) → Scan)',
      guideChoice: '· If asked to pick an app on the phone: choose "Connect existing app" if this bot was created before, otherwise "Create new app"',
      guideWhy: '· The confirmation step on your phone is part of the Feishu open-platform authorization flow',
    ...
    imBots: {
      ...
      dingtalk: { step1: 'DingTalk Open Platform (open-dev.dingtalk.com) → Apps → Create an internal app',
                  step2: 'Add the "Robot" capability, choose "Stream Mode" for message receiving, then publish the app',
                  step3: 'Copy AppKey / AppSecret from "Credentials & Basic Info" into the form above' },
      wecom: { step1: 'WeCom admin console → Security & Management → Management Tools → AI Bots',
               step2: 'Create bot: Manual → API mode → Long connection (WebSocket)',
               step3: 'Copy BotId / Secret from the bot detail page into the form above' },
    },
    llm: {
      title: 'AI Models',
      budget: 'Budget',
