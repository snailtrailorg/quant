...
//   fmtTime.day(ts) → 'YYYY-MM-DD'         到日（批 56b：API ts 改 UTC ISO 表示后
//                                          字符串 slice 会错位 8h/跨日——一律本地解析取日）
...
export const fmtTime = {
  ...
  day: (ts) => { const d = norm(ts); return d instanceof Date ? `${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}` : d },
}
