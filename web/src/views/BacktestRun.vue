  ...
  <el-card v-loading="loading">
    ...
    <el-card>
      ...
      <TableShell :data="symbols" @row-click="goView" storage-key="backtest-run-symbols">
        ...
        <el-table-column v-if="colOn('volatility')" prop="volatility" :label="t('cols.volatility')" min-width="120" class-name="num">
          <template #default="{ row }">{{ row.result?.volatility ?? '—' }}</template>
        </el-table-column>
        <el-table-column v-if="colOn('win_rate')" prop="win_rate" :label="t('cols.winRate')" min-width="101" class-name="num">
          <template #default="{ row }">{{ row.result?.win_rate != null ? row.result.win_rate + '%' : '—' }}</template>
        </el-table-column>
        <el-table-column v-if="colOn('max_drawdown')" prop="max_drawdown" :label="t('cols.maxDrawdown')" min-width="138" class-name="num">
...
.stat .label { color: var(--text-secondary); font-size: var(--fs-label); }
.stat .value { font-size: var(--fs-kpi); font-weight: bold; color: var(--text-primary); margin-top: 4px; }
</style>
