  ...
  <el-card>
    ...
    <TabsShell :tabs="tabs" default-tab="nav" v-slot="slotProps">
      ...
      <template v-if="slotProps.tab === 'nav'">
        <TableShell :data="rows.nav" storage-key="perm-res-nav">
          <el-table-column prop="id" :label="t('permRes.colRes')" min-width="140" />
          <el-table-column prop="group" :label="t('permRes.colGroup')" min-width="100" />
      ...
      <template v-else-if="slotProps.tab === 'api'">
        ...
        <TableShell :data="rows.api" storage-key="perm-res-api">
          <el-table-column prop="key" :label="t('permRes.colRes')" min-width="180">
            <template #default="{ row }">
              {{ row.key }}
              <span v-if="row.locked" :title="t('permRes.locked')"> 🔒</span>
            </template>
          </el-table-column>
          ...
          <el-table-column :label="t('permRes.colStatus')" width="90">
            <template #default><el-tag type="info" size="small">{{ t('permRes.readonly') }}</el-tag></template>
          </el-table-column>
        </TableShell>
      </template>
      <!-- 市场键页签（只读） -->
    ...
    <el-dialog v-model="editDlg" :close-on-click-modal="false" :title="t('permRes.editTitle')" width="440px">
      <el-form label-width="110px">
        <el-form-item :label="t('permRes.colRes')"><el-input :model-value="editing.id" disabled /></el-form-item>
